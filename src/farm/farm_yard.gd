...
func build() -> void:
	...
	_add_ground()
	_add_arena()
	## 3/4 barn: painted aisle floor is well above the lowest posts — sink it.
	_add_sprite(SPR_BARN, Vector3(-8.5, 0, -6.0), 7.4, 1.65)
	_add_sprite(SPR_JUMP, Vector3(6.5, 0, -2.4), 1.7, 0.08)
	_add_sprite(SPR_JUMP, Vector3(7.4, 0, 6.2), 1.55, 0.08)
	for i in 5:
		_add_sprite(SPR_FENCE, Vector3(-14.0 + float(i) * 3.1, 0, 6.8), 1.25, 0.06)
	_add_sprite(SPR_TREE, Vector3(-17.0, 0, -9.0), 8.5, 0.45)
	_add_sprite(SPR_TREE, Vector3(17.5, 0, -7.0), 7.6, 0.4)
	_add_sprite(SPR_TREE, Vector3(-16.0, 0, 10.0), 7.2, 0.4)
	_add_sprite(SPR_TREE, Vector3(15.0, 0, 12.0), 6.8, 0.35)
...
func _add_sprite(tex: Texture2D, origin: Vector3, height_m: float, sink: float = 0.0) -> void:
	var s := SpriteProp.make(tex, height_m, sink)
